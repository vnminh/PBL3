...
<title>${requestScope.product.ten}</title>

<!-- Custom Scripts -->
<script src="${pageContext.request.contextPath}/js/toast.js"
	type="module"></script>
			...
			<div class="row">
				...
				<aside
					...
					<img width="280" height="280" class="img-fluid"
								src="${requestScope.product.urlImage}"
								alt="${requestScope.product.urlImage}">
				</aside>

				<main class="col-md-7">

					<h2 class="title">${requestScope.product.ten}</h2>

					<div class="rating-wrap my-3">
						<span class="rating-stars me-2"> <c:forEach begin="1"
								end="5" step="1" var="i">
								<i
									class="bi bi-star-fill ${i <= requestScope.product.soSaoTB ? 'active' : ''}"></i>
							</c:forEach>
					...
					<div class="mb-4">
						<c:choose>
							<c:when test="${requestScope.product.phanTramGiamGia == 0}">
								<span class="price h4"> <fmt:formatNumber pattern="#,##0"
										value="${requestScope.product.giaBan}" />₫
								</span>
							</c:when>
							<c:otherwise>
								<span class="price h4"> <fmt:formatNumber pattern="#,##0"
										value="${requestScope.product.giaBan * (100 - requestScope.product.phanTramGiamGia) / 100}" />₫
								</span>
								...
								<span class="ms-2 badge bg-info"> -<fmt:formatNumber
										pattern="#,##0" value="${requestScope.product.phanTramGiamGia}" />%
								</span>
							</c:otherwise>
						</c:choose>
					</div>
					...
					<c:if test="${requestScope.totalProductReviews != 0}">
						...
						<div class="rattings-wrapper mb-5">
							<c:forEach var="productReview"
								items="${requestScope.productReviews}">
								<div class="sin-rattings mb-4">
									<div class="star-author-all mb-2 clearfix">
										<div class="ratting-author float-start">
											...
											<span> <fmt:parseDate
													value="${productReview.createdAt}"
													pattern="yyyy-MM-dd'T'HH:mm" var="parsedCreatedAt"
													type="both" /> <fmt:formatDate pattern="HH:mm dd/MM/yyyy "
                      ...
                      ${productReview.content}
									...
									<c:if
										test="${productReview.userId == sessionScope.currentUser.id}">
										<form
											action="${pageContext.request.contextPath}/deleteProductReview"
											method="post">
											<input type="hidden" name="productReviewId"
												value="${productReview.id}"> <input type="hidden"
												name="productId" value="${requestScope.product.id}">
											<div class="btn-group" role="group">
												<a
													href="${pageContext.request.contextPath}/editProductReview?id=${productReview.id}"
													role="button" class="btn btn-primary btn-sm"> Sửa </a>
							...
							<nav class="mt-4">
								<ul class="pagination">
									<li
										class="page-item ${requestScope.pageReview == 1 ? 'disabled' : ''}">
										<a class="page-link"
									...
									<c:forEach begin="1"
										end="${requestScope.totalPagesOfProductReviews}" var="i">
										<c:choose>
									...
									<li
										class="page-item ${requestScope.pageReview == requestScope.totalPagesOfProductReviews ? 'disabled' : ''}">
										<a class="page-link"
										href="${pageContext.request.contextPath}/product?id=${requestScope.product.id}&pageReview=${requestScope.pageReview + 1}#review">
											Trang sau </a>
									</li>
						...
						<c:when test="${not empty sessionScope.currentUser}">
							<div class="ratting-form-wrapper">
								<div class="ratting-form">
									<form
										action="${pageContext.request.contextPath}/addProductReview"
										method="post">
										<div class="row">
											<div class="col-md-3 mb-3">
												<select
													class="form-select ${not empty sessionScope.violations.ratingScoreViolations
                                ? 'is-invalid' : (not empty sessionScope.values.ratingScore ? 'is-valid' : '')}"
													name="ratingScore">
													<option disabled ${not emptysessionScope.values.ratingScore ? '' : 'selected'}>
														Cho sao</option>
													<c:forEach var="i" begin="1" end="5">
														<option value="${i}"
															${sessionScope.values.ratingScore == i ? 'selected' : ''}>${i}</option>
													</c:forEach>
												</select>
												<c:if
										...
										<div class="row mb-3">
											<div class="col">
												<textarea
													class="form-control ${not empty sessionScope.violations.contentViolations
                                  ? 'is-invalid' : (not empty sessionScope.values.content ? 'is-valid' : '')}"
													name="content" placeholder="Nội dung đánh giá" rows="3">${sessionScope.values.content}</textarea>
												<c:if
													test="${not empty sessionScope.violations.contentViolations}">
													<div class="invalid-feedback">
														<ul class="list-unstyled mb-0">
															<c:forEach var="violation"
					...
					<div class="col-xl-3 col-lg-4 col-md-6">
						<div class="card p-3 mb-4">
							<a
								...
								class="img-wrap text-center"> <c:choose>
									...
									<c:choose>
										<c:when test="${relatedProduct.discount == 0}">
											<span class="price mt-1 fw-bold"> <fmt:formatNumber
													pattern="#,##0" value="${relatedProduct.price}" />₫
											</span>
										</c:when>
										<c:otherwise>
											<span class="price mt-1 fw-bold"> <fmt:formatNumber
											...
											<span class="ms-2 badge bg-info"> -<fmt:formatNumber
													pattern="#,##0" value="${relatedProduct.discount}" />%
											</span>
										</c:otherwise>
									</c:choose>
								</div>
